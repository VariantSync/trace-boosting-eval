...
def parseDataRQ1(data):
    ...
    for l2, l2_data in l3.items():
        ...
        accuracy_df = pd.DataFrame(accuracy_data, index=list(l2_data.values())[0].keys()).T
        precision_df = pd.DataFrame(precision_data, index=list(l2_data.values())[0].keys()).T
        recall_df = pd.DataFrame(recall_data, index=list(l2_data.values())[0].keys()).T
        f1_score_df = pd.DataFrame(f1_data, index=list(l2_data.values())[0].keys()).T
        
        dataInSamples[l2]= {'accuracy_df':accuracy_df, 'precision_df':precision_df, 'recall_df':recall_df, 'f1_score_df':f1_score_df}
...
def plotLines(experiment, dataset_name, sample_size, average_df, accuracy_df, precision_df, recall_df, f1_score_df, marginAlpha=0.3, title='Test', rq=1):
    ...
    # Plot accuracy
    if plot_accuracy:
        plot_dataframe('average accuracy improvement', accuracy_df, color[0], interpolation, marginAlpha, x, x_smooth)

    # Plot precision
    if plot_precision:
        plot_dataframe('average precision improvement', precision_df, color[1], interpolation, marginAlpha, x, x_smooth)

    # Plot recall
    if plot_recall:
        plot_dataframe('average recall improvement', recall_df, color[2], interpolation, marginAlpha, x, x_smooth)
        # Plot the investment line
        # plt.plot([0, 5, 10, 15, 20, 25], [0, 5, 10, 15, 20, 25], linestyle='--', label='1-to-1 return on investment')

    # # Plot f1
    if plot_f_score:
        plot_dataframe('average f1 improvement', f1_score_df, color[3], interpolation, marginAlpha, x, x_smooth)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(fname='../results/plots/relative/' + dataset_name + '_lineplot_' + sample_size, dpi='figure', format=None)
    plt.show()
...
def plot_dataframe(label, accuracy_df, color, interpolation, marginAlpha, x, x_smooth):
    ...
    plt.fill_between(x_smooth, y1th_smooth, y2th_smooth, color=color, alpha=marginAlpha)
...
def plotBoxs(data, title, ylabel, filename="Test", rq=1):
    ...
    plt.savefig(fname='../results/plots/final/' + filename + ".pdf", dpi='figure', format="pdf")
    plt.show()
...
def main():
    ...
    for datasetName in subjects:
        ...
        for sample_size, sampleData in data.items():
            ...
            # boxplots
            title = ''
            plotBoxs(sampleData['accuracy_df'], title=title, ylabel='Accuracy', filename="accuracy/" + datasetName + '-' + sample_size)
            plotBoxs(sampleData['precision_df'], title=title, ylabel='Precision', filename="precision/" + datasetName + '-' + sample_size)
            plotBoxs(sampleData['recall_df'], title=title, ylabel='Recall', filename="recall/" + datasetName + '-' + sample_size)
            plotBoxs(sampleData['f1_score_df'], title=title, ylabel='F1 Score', filename="f1_score/" + datasetName + '-' + sample_size)
            print('------------------------------------------------------------------')
