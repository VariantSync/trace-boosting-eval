...
def parseDataRQ1(data):
    ...
    for l2, l2_data in l3.items():
        ...
        for l1, l1_data in l2_data.items():
            ...
            avg_accuracy = compute_average(l1_data, "accuracy")#sum(run_data["accuracy"] for run_data in l1_data.values()) / length
            avg_precision = compute_average(l1_data, "precision")# sum(run_data["precision"] for run_data in l1_data.values()) / len(l1_data)
            avg_recall = compute_average(l1_data, "recall")  #sum(run_data["recall"] for run_data in l1_data.values()) / len(l1_data)
            avg_f1_score = compute_average(l1_data, "f1-score") #sum(run_data["f1-score"] for run_data in l1_data.values()) / len(l1_data)
...
for sample_size, sampleData in dataInSamplesrq1.items():
    ...
    # averages
    plotBars(sampleData['average_df'].T,    title=datasetName + ' ' + sample_size)

    # boxplots
    plotBoxs(sampleData['accuracy_df'],     title=datasetName + '-' + sample_size + '-accuracy')
    plotBoxs(sampleData['precision_df'],    title=datasetName + ' ' + sample_size + '-precision')
    plotBoxs(sampleData['recall_df'],       title=datasetName + ' ' + sample_size + '-recall')
    plotBoxs(sampleData['f1_score_df'],     title=datasetName + ' ' + sample_size + '-f1_score')
    print('------------------------------------------------------------------')
...
for percentage_size, percentageData in dataInSamplesrq2.items():
    ...
    plotBars(percentageData['average_df'].T,    title=datasetName + '-' + percentage_size, rq=2)
    plotBoxs(percentageData['accuracy_df'],     title=datasetName + '-' + percentage_size + '-accuracy', rq=2)
    plotBoxs(percentageData['precision_df'],    title=datasetName + '-' + percentage_size + '-precision', rq=2)
    plotBoxs(percentageData['recall_df'],       title=datasetName + '-' + percentage_size + '-recall', rq=2)
    plotBoxs(percentageData['f1_score_df'],     title=datasetName + '-' + percentage_size + '-f1_score', rq=2)
    print('------------------------------------------------------------------')
